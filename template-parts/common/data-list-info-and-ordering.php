<?php
global $order_option, $found_posts, $post_type_multiple;
?>
...
<div class="d-flex align-items-center justify-content-between" data-current-order="<?php echo esc_attr($order_option); ?>">
                <p id="autocomplete-label" class="u-grey-light text-paragraph-card mt-4 mb-4 mt-lg-30 mb-lg-30 mb-0 mt-0 pe-2">
                    <span class="badge rounded-pill bg-primary"> <?= $found_posts ?> </span> <?php echo $post_type_multiple; ?> trovati in <strong id="current-order-text">
                    <?php
                        if ($order_option === "date_desc") {
                            echo "ordine di pubblicazione decrescente";
                        } else if ($order_option === "date_asc") {
                            echo "ordine di pubblicazione crescente";
                        } else if ($order_option === "post_title_desc") {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="filterOption" id="opt-post_title_asc" value="post_title_asc" <?= $order_option === "post_title_asc" ? "checked" : "" ?>>
                            <label class="form-check-label" for="opt-post_title_asc">Ordine alfabetico (A-Z)</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="filterOption" id="opt-post_title_desc" value="post_title_desc" <?= $order_option === "post_title_desc" ? "checked" : "" ?>>
                            <label class="form-check-label" for="opt-post_title_desc">Ordine alfabetico inverso (Z-A)</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="filterOption" id="opt-date_asc" value="date_asc" <?= $order_option === "date_asc" ? "checked" : "" ?> >
                            <label class="form-check-label" for="opt-date_asc">Ordine di pubblicazione crescente</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="filterOption" id="opt-date_desc" value="date_desc" <?= $order_option === "date_desc" ? "checked" : "" ?> >
                            <label class="form-check-label" for="opt-date_desc">Ordine di pubblicazione decrescente</label>
                        </div>
                    </div>
